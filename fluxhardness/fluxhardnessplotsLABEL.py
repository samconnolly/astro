...
# file names
energysoftfname 	= objectname+"_lcurve_4_gti_0.5-2keV.qdp"
energyhardfname		= objectname+"_lcurve_4_gti_2-10keV.qdp"
...
def hardnessflux(tmin,tmax):
	...
	# sort labels, modified hardnesses and soft flux according to hard flux
		...
		if start !=0:
			for comp in range(len(modfluxhard[0])): # for each value 
				...
				if hardflux[value] <= modfluxhard[0][comp]:# add 
					...
					modfluxhard[6].insert(comp,labels[value])
					break
				if comp == len(modfluxhard[0]) -1:  # add after last 
					...
					modfluxhard[6].append(labels[value])
...
if labelsOn:
	...
	for index in range(len(modfluxhard[6])):
		plt.annotate(modfluxhard[6][index], 

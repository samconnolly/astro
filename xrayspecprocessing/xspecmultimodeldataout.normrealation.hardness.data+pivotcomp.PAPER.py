...
factor  = 1.174E-15 # b for unabs PL
index   = 4.471  # a
factor2 = 5.691E-6 # b for abs PL
index2  = 2.913  # a
...
varstart  = 0.5 # lower value in range for variable parameter
varend    = 10. # upper value in range for variable parameter
...
norm1		= '4.44366E-04' #'[expr pow($var,'+str(index)+')*'+str(factor)+']'
pow2		= True			# absorbed
norm2		= '1.54128E-03' #'[expr pow($var,'+str(index2)+')*'+str(factor2)+']'

# gaussian
gauss		= True
gaussE		= "0.8"
gaussS		= "0.1"
gaussnorm	= "1.34985E-04"
...
if graph:
	...
	for p in [0,10,19]:
		...
		plt.plot(energy[p],flux[p],linewidth = 2)
	...
	plt.plot(hard, hardness, marker='o',linewidth = 2, color = 'blue')
	if domodel:
	...
	plt.errorbar(modfluxhard[0],modfluxhard[2],	xerr = modfluxhard[1],  \
			yerr = modfluxhard[3], marker='.', color = 'red', \
				ecolor = 'grey', linestyle = 'none',capsize = 0)
	plt.plot(hard, soft, marker='o',linewidth = 2, color = 'blue')
	if domodel:

...
outroute = "/disks/raid/raid1/xray/raid/sdc1g08/NetData/ngc1365/spectra/gtithinbin/"

# output data file name
outfile  = "multimodelhardness.dat"
specfile = "0.16-0.208.summed.pha"

# number of model spectra to extract
specnum = 10 # spectrum number of model to extract
...
factor  = 0.000538 # b for unabs PL
index   = -.245  # a
factor2 = 0.01555 # b for abs PL
index2  = -.245  # a
...
varstart  = .01 # lower value in range for variable parameter
varend    = 999.99 # upper value in range for variable parameter

logvar =  False # spread the varying paramter's values logorithmically?
...
gaussE		= "0.8"
gaussS		= "0.1"
gaussnorm	= "1.07663E-04"

#ini
# cold absorption
wabs 		= False
nH 			= '5'#'$var'#'[expr exp($var)]' # 10^22

#absori
absori		= True
nH			= '5'#'$var'#'[expr exp($var)]'#'$var' # 
Tabs		= "3E+4"
ion			= '$var'#"0.262149"
z			= "0.005569"
Fe			= "2.8"
...
tmplocationenergy  = outroute + tmpenergy
tmplocationflux  	= outroute + tmpflux
...
ignore 10.-**\n\
notice 0.5-10.0\n\
set outfile2 [open {0}$i.dat w]\n\
tclout energies 1\n\
puts $outfile2 $xspec_tclout\n\
set outfile2 [open {1}$i.dat w]\n\
tclout modval 1\n\
puts $outfile2 $xspec_tclout\n\
tclout rate 1\n\
...
puts $outfile $xspec_tclout \n\
puts $xspec_tclout \n\
set var [expr $var + $increment]'.format(tmpenergy,tmpflux)
...
tmplocation = tmplocationenergy + str(1) + ".dat"

energy = open(tmplocation, 'r')
...
for line in energy:
	...
	for e in linedata:
		...
		if float(e)>=0.5 and float(e)<=10.0:
...
for y in range(specnum):
	...
	tmplocation = tmplocationflux + str(y+1) + ".dat"

	fluxes = open(tmplocation, 'r')
	...
	for line in fluxes:
		...
		for f in nums:
			...
			flux[y].append(float(linedata[f]))
...
if graph:
	...
	plt.xlabel("Hard Count Rate ($s^{-1}$)")
	...
	plt.ylabel("Soft Count Rate ($s^{-1}$)")
	plt.xlabel("Hard Count Rate ($s^{-1}$)")

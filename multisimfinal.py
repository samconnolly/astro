...
makes no difference of course. The 'X-ray' data set is simulated.
...
xrayfname   = "XS.dat"		# xray data filename
outfname    = "xcor.dat"	# output data filename

# variables
n_bins  =  500			# number of bins to bin CCF
ncurves =  10000			# number of simulated lightcurves
...
# plots to show
autocorr = False	# autocorrelation functions of each dataset
...
for n in xtime:
	...
	simtime[0].append( int(n - xtime[0]) )	# integer time since start time
	simtime[1].append( n - int(n) )			# time difference to actual value
...
for curve in range(ncurves):
	...
	lc  = lcsim(psdindexlow, psdindexhigh, psdbrkfreq,
	...
	sxc, stl = xcor(rflux, rtime, lcsamp, xtime,  n_bins)
...
rxc, rtl = xcor(rflux, rtime, rflux, rtime, n_bins)
xxc, xtl = xcor(xflux, xtime, xflux, xtime, n_bins)
...
# sample lightcurves and data lightcurve
if lcomp:
...
# data ccf with confidence curves
if confccf:

...
#================================================================================
# Discrete cross correlation and confidence-simulation code BINWISE
#================================================================================
...
makes no difference of course. The 'X-ray' data set is simulated. Incorporates errors, calculates satas binwise to account for trends in
lightcurves.

Requires: 
	lcsimulation		- light curve simulation code (Timmer & Koenig method)
	callxcorErrBinwise	- discrete cross-correlation code
"""

# Import packages

from lcsimulation import lcsim
from callXcorErrBinwise import xcor
from pylab import *
import numpy as np
...
radioroute = "/uvot/"											# radio path
xrayroute  = "/uvot/"											# xray path
outroute   = "/uvot/"											# output path

# file names
radiofname  = "V.dat"		# radio data filename
xrayfname   = "XCounts.dat"		# xray data filename
outfname    = "xcor.dat"	# output data filename

# variables
n_bins  =  130			# number of bins to bin CCF
...
# - radio -
# right edge
rhigh = rxc.index(max(rxc[int(len(rxc)/4):int(3*len(rxc)/4)])) # start from highest point
clow = max(rxc[int(len(rxc)/4):int(3*len(rxc)/4)])# set lowest value so far to highest value
print len(rxc), rhigh
...
while  (rxc[rhigh+1] < clow) or (rxc[rhigh+2] < clow) or (rxc[rhigh+3] < clow):
	print clow
	m = rxc[rhigh+1:rhigh+4]			# create list of next three avlues
	clow = min(m)						# find minimum of next three values
	rhigh = m.index(clow) + rhigh + 1	# set new position to lowest value's


# left edge
rlow = rxc.index(max(rxc[int(len(rxc)/4):int(3*len(rxc)/4)]))
clow = max(rxc[int(len(rxc)/4):int(3*len(rxc)/4)])
...
rhm = (max(rxc[int(len(rxc)/4):int(3*len(rxc)/4)]) - (rxc[rhigh] + rxc[rlow])*0.5)*0.5 +(rxc[rhigh] + rxc[rlow])*0.5

# - xray -
# right edge
xhigh = xxc.index(max(xxc[int(len(xxc)/4):int(3*len(xxc)/4)]))
clow = max(xxc[int(len(xxc)/4):int(3*len(xxc)/4)])

while  (xxc[xhigh+1] < clow) or (xxc[xhigh+2] < clow) or (xxc[xhigh+3] < clow):

	m = xxc[xhigh+1:xhigh+4]
	clow = min(m)
	xhigh = m.index(clow) + xhigh + 1

# left edge
xlow = xxc.index(max(xxc[int(len(xxc)/4):int(3*len(xxc)/4)]))
clow = max(xxc[int(len(xxc)/4):int(3*len(xxc)/4)])
...
# half maximum - using average low value and highest value - DISTANCE ABOVE ZERO
xhm = (max(xxc[int(len(xxc)/4):int(3*len(xxc)/4)]) - (xxc[xhigh] + xxc[xlow])*0.5)*0.5 +(xxc[xhigh] + xxc[xlow])*0.5
...
lowhm = rxc.index(max(rxc[int(len(rxc)/4):int(3*len(rxc)/4)]))
...
highhm = rxc.index(max(rxc[int(len(rxc)/4):int(3*len(rxc)/4)]))
...
# lower half maximum position
lowhm = xxc.index(max(xxc[int(len(xxc)/4):int(3*len(xxc)/4)]))
...
# upper half maximum position
highhm = xxc.index(max(xxc[int(len(xxc)/4):int(3*len(xxc)/4)]))
...
# plot data ccf with confidence curves, number of data points in each bin
if confccf:
		
	title("Cross correlation function of data with confidence curves")
	subplot(2,1,1)

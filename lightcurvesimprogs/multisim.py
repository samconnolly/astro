...
xrayfname   = "U.dat"		# xray data filename
outfname    = "xcor.dat"	# output data filename

# variables
n_bins  =  200			# number of bins to bin CCF
ncurves =  2			# number of simulated lightcurves
t_lim   = True			# limit time lag range?
t_range =  150			# time range (+ and -) for lags in days	
...
for curve in range(ncurves):
	...
	sxc, stl, sba = xcor(rflux, rtime, lcsamp, xtime,  n_bins,t_lim,t_range)
...
rxc, rtl, rbm = xcor(rflux, rtime, rflux, rtime, n_bins,t_lim,t_range)
xxc, xtl, xbm = xcor(xflux, xtime, xflux, xtime, n_bins,t_lim,t_range)

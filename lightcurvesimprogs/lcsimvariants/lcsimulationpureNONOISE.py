...
def lcsim(low,high,brk,seed,sd,mean,lg):
	...
	powerlaw = numpy.piecewise(frequency,  [ frequency <= breakfreq, frequency>=breakfreq] , 
        	   [lambda frequency: (frequency)**(-psdindexLow), 		
        	    lambda frequency: ((frequency)**(-psdindexHigh))*((breakfreq)**(psdindexHigh-psdindexLow)) ])
	...
	real = (numpy.sqrt(powerlaw*0.5))
	imag = (numpy.sqrt(powerlaw*0.5))
	...
	longlightcurve = inversefourier.real             # take real part of the transform (remove zeroes)
	...
	return normlightcurve,powerlaw,positive

...
def lcsim(incurve,intimes,psdindexLow,psdindexHigh,breakfreq,mean,sd,randomSeed):
	...
	Arguments:
		incurve:
			(array)	The lightcurve to be imitated
		intimes:
			(array)	The corresponding time measurements
		psdindexLow:
			(float)	spectral index of low frequency end of PSD
		psdindexHigh:
			(float)	spectral index of high frequency end of PSD
		breakfreq:
			(float)	break frequency of PSD
		mean:
			(float)	mean amplitude (cnts/flux) of lightcurve to generate
		std:
			(float)	standard deviation of amplitude of lightcurve to generate
		randomSeed:
	...
	lclength		= max(intimes) - min(intimes)
	while length < lclength:	# double length until longer than 
		length *= 2

...
# UNEVEN BINS.
# THE HARD + TOTAL VERSION will take the binning values from one file (hard cnts)
# and the min. values (total cnts) from another, so that the total spectra
# end up having a decent S/N
#===============================================================================
"""
...
bincolumn = 6 # column to bin along in bin data file

exposurecolumn = 3 # exposure time column in bin data file

mincount = True # minimum total for some value in each bin?
mincolumn = 12  # min column in min data file
binmin = 2000
...
# file name
binfilename 	= "NGC1365_lcurve_gti_2-10keV.txt"
minfilename   = "NGC1365_lcurve_gti_0.5-10keV.txt"
...
# create file routes
binlocation  = route+binfilename
minlocation  = route+minfilename
savelocation  = outroute+savefile

# read bin data intoarray

start = 0

infile= open(binlocation, 'r')
...
# read min data intoarray

start = 0

infile2= open(minlocation, 'r')
...
for line in infile2:
	...
	if start == header:
		columns = len(linedata)
		mdata = [[] for x in range(columns + 1)]
		...
		if linedata[10] == 'a1':
			...
			for column in range(columns):
				if len(linedata) == columns:
					...
					try:
						mdata[column].append(float(linedata[column]))
					except ValueError:
						mdata[column].append(linedata[column])
...
for index in range(len(sortindex)):
	...
	total += mdata[mincolumn][sortindex[index]] # add counts

...
# broken x-axis. plus y errors. And a highlighted area, which is magnified in
# a seperate plot.
#===============================================================================
...
xlabel = "MJD - 50,000"
ylabel = "0.5-10.0 keV Count Rate (counts s$^{-1}$)"
toplabel = ''

#   File routes
route   = "/net/raid/raid1/xray/raid/sdc1g08/NetData/ngc1365/lightcurve/\
				refinedCounts/"
...
# expanded region, highlighted on first plot and plotted in second

explims = [6220,6330]

expxlabel   = "MJD - 50,000"
expylabel   = "0.5-10.0 keV Count Rate (counts s$^{-1}$)"
exptoplabel = ''
...
fig.text(.02, .80, ylabel, fontsize = 14, fontweight = 'bold',rotation = 90)
...
plt.errorbar(data[xcolumn],data[ycolumn], yerr= data[yerrcolumn],\
			marker='o', color = 'red', \
			ecolor = 'grey', linestyle = 'none',capsize = 0,linewidth=2)
#plt.xlabel(expxlabel)
#plt.ylabel(expylabel)
...
fig.text(.45, .02, expxlabel, fontsize = 14, fontweight = 'bold')
fig.text(.02, .80, expylabel, fontsize = 14, fontweight = 'bold',rotation = 90)
